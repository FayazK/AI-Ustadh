# Tadabbur Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create the `/tadabbur` skill — deep investigative Quranic exploration through relentless "why, how, when, what" questioning in a unified scholarly lecture voice.

**Architecture:** New skill at `.claude/skills/tadabbur/SKILL.md` following the same agent-dispatch, sequential-parts, compile-to-PDF pattern used by divine/discover/recite. Also update CLAUDE.md and README.md to register the new command.

**Tech Stack:** Claude Code skill (markdown), md-to-pdf (Node.js/weasyprint), research skill (yt-dlp, youtube-transcript-api)

**Spec:** `docs/superpowers/specs/2026-04-01-tadabbur-skill-design.md`
...
|--------|------|---------------|
| Create | `.claude/skills/tadabbur/SKILL.md` | Complete skill definition |
| Modify | `CLAUDE.md` | Add tadabbur to Commands and Skills sections |
| Modify | `README.md` | Add tadabbur skill section |

---

### Task 1: Create the Tadabbur Skill

**Files:**
- Create: `.claude/skills/tadabbur/SKILL.md`

This is the core deliverable. The skill file follows the same structure as `.claude/skills/divine/SKILL.md` and `.claude/skills/discover/SKILL.md` — YAML frontmatter, overview, constraints, input format, section definitions, execution flow, agent dispatch steps, language rules, and important notes.

- [ ] **Step 1: Write the SKILL.md file**

Create `.claude/skills/tadabbur/SKILL.md` with the following complete content:

```markdown
---
name: tadabbur
description: Use when the user invokes /tadabbur with any Quranic verse, surah, hadith, Islamic concept, event, story, or practice. Deep investigative exploration through relentless questioning — why this word, why not another, why here, why now — delivered as a unified scholarly lecture, compiled into a PDF.
---
...
digraph tadabbur_flow {
    ...
    context [label="Step 3: Agent — Context\n(01-context.md)"];
    investigation [label="Step 4: Agent — Investigation\n(02-investigation.md)"];
    connections [label="Step 5: Agent — Connections\n(03-connections.md)"];
    depths [label="Step 6: Agent — Depths\n(04-depths.md)"];
...
Run: `head -5 .claude/skills/tadabbur/SKILL.md && wc -l .claude/skills/tadabbur/SKILL.md`

Expected: YAML frontmatter visible, approximately 280-320 lines total.

- [ ] **Step 3: Commit**

```bash
git add .claude/skills/tadabbur/SKILL.md
git commit -m "feat: add tadabbur skill — deep investigative Quranic exploration"
```
...
Add tadabbur to both the Commands and Skills sections, maintaining alphabetical order within each section.

- [ ] **Step 1: Add tadabbur to the Commands section**

In `CLAUDE.md`, after the `/discover` command entry and before the `/recite` command entry, add:
...
The Commands section should now read (in this order):
1. `/discover`
2. `/divine`
3. `/recite`
4. `/research`
...
Wait — the existing order is: `/recite`, `/divine`, `/discover`, `/research`. Insert `/tadabbur` at the end, after `/research`:

```markdown
- `/tadabbur [verse, surah, event, concept, or topic]` — Deep investigative exploration through relentless "why, how, when, what" questioning, delivered as a unified scholarly lecture — compiled into a PDF.
```

- [ ] **Step 2: Add tadabbur to the Skills section**

In `CLAUDE.md`, after the `research` skill entry and before the `md-to-pdf` entry, add:
...
Run: `grep -n "tadabbur" CLAUDE.md`

Expected: Two matches — one in Commands section, one in Skills section.

- [ ] **Step 4: Commit**

```bash
git add CLAUDE.md
git commit -m "docs: add tadabbur to CLAUDE.md commands and skills"
```
...
Add a tadabbur section to the README following the same pattern as the other skill sections (description, example commands, what it produces, output structure).

- [ ] **Step 1: Add the tadabbur skill section**

In `README.md`, after the `/discover` section's closing `---` and before the `/research` section, add:
...
Note: The triple backticks above are escaped with spaces for plan readability. Use actual triple backticks (```) in the file.

- [ ] **Step 2: Update the "Use" section**

In the "Use" section near the bottom of README.md, add tadabbur to the example commands:
...
And in the "All input formats" section, add:
...
Run: `grep -c "tadabbur" README.md`

Expected: At least 8 matches (section header, examples, output structure, use section).

- [ ] **Step 4: Commit**

```bash
git add README.md
git commit -m "docs: add tadabbur skill to README"
```
